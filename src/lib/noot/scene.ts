...
export function mountNoot(
  ...
  room.dispose();
  pmrem.dispose();
  const fill = new THREE.HemisphereLight("#f7f4e8", "#747564", 0.7);
  scene.add(fill);
  ...
  gradient.addColorStop(0, "rgba(29,40,13,.24)");
  gradient.addColorStop(0.5, "rgba(29,40,13,.1)");
  gradient.addColorStop(1, "rgba(29,40,13,0)");
  ...
  function render(timestamp: number) {
    ...
    if (state.onRuler && state.pose === "skip") {
      ...
      floorOrigin.set(0, 0, 0).project(camera);
      floorUnit.set(1, 0, 0).project(camera);
      const pixelsPerUnit =
        (Math.abs(floorUnit.x - floorOrigin.x) * canvasWidth) / 2;
      // Use ruler-relative position: scrolling the page must not create footsteps.
      const rulerLeft =
        canvas.closest(".stage-ruler")?.getBoundingClientRect().left ?? 0;
      const localLeft = rect.left - rulerLeft;
      if (previousLeft !== undefined && pixelsPerUnit > 0)
        travelSpeed =
          (localLeft - previousLeft) /
          Math.max(animationDt, 0.001) /
          pixelsPerUnit;
      previousLeft = localLeft;
    } else previousLeft = undefined;
    noot.update(
      elapsed,
      state.paused ? 0 : animationDt,
      { ...state, travelSpeed },
      pointer,
      media.matches,
    );
    ...
    if ((!media.matches && !state.paused) || timestamp < settleUntil)
