...
type Mode = 'lobby'|'playing'|'perfect'|'tie'|'finished'|'timeout'|'spectator'
...
export default function MatchPreview() {
  ...
  const entries: MatchEntry[] = names.slice(0,count).map((name,i)=>({id:String(i),name,points:Math.max(0, [12500,12000,8500][i] ?? 8000-i*500),stage:i===0?stage:1,status:i===0&&out?'out':'playing',lastAction:i===0?action:'ready',delta:0,history:[5000,4000,3500]}))
  if (mode==='perfect'||mode==='tie'||mode==='finished') entries.forEach((e,i)=>{e.status='solved';e.lastAction='solved';e.delta=[5000,4000,3000][i] ?? 1000;e.points=[17500,16000,11500][i] ?? 9000-i*500;e.solvedAt=epoch+i*5000})
  ...
  const match: MatchView|null = mode==='lobby'?null:{scoringVersion:2,id:'preview',roundId:'round-'+mode+'-'+epoch,number:mode==='finished'?10:4,phase:mode==='playing'||mode==='spectator'?'playing':mode==='finished'?'finished':'reveal',difficulty,length:10,startsAt:epoch,deadline:epoch+90000,entries:mode==='spectator'?entries.slice(1):entries,audio,offset:0,answer:mode==='playing'||mode==='spectator'?null:{id:'preview-song',title:'A perfect little moment',artist:'Original preview melody',albumArt:''}}
  ...
  return <><div className="match-preview-controls"><strong>Local preview · generated melody</strong><select aria-label="Preview players" value={count} onChange={e=>setCount(Number(e.target.value))}>{[2,3,6,12].map(n=><option key={n} value={n}>{n} players</option>)}</select>{(['lobby','playing','perfect','tie','finished','timeout','spectator'] as const).map(m=><button key={m} aria-pressed={mode===m} onClick={()=>{setMode(m);setStage(0);setAction('ready');setOut(false);setReady(false);setEpoch(Date.now())}}>{m}</button>)}<button onClick={()=>setScoreBoost(v=>({...v,1:(v[1]??0)+2000}))}>Milo +2,000</button><button onClick={()=>setScoreBoost(v=>({...v,0:(v[0]??0)+2000}))}>You +2,000</button><button onClick={()=>setScoreBoost({})}>Reset scores</button><button onClick={()=>setTheme(t=>t==='light'?'dark':'light')}>Theme</button><button onClick={()=>setProfile(true)}>Wardrobe</button><button aria-pressed={!connected} onClick={()=>setConnected(c=>!c)}>Offline</button><select aria-label="Preview difficulty" value={difficulty} onChange={e=>setDifficulty(e.target.value as Difficulty)}>{(['easy','medium','hard','expert','impossible'] as const).map(d=><option key={d}>{d}</option>)}</select></div><MatchArena table={table} theme={theme}/>{profile&&<NootProfile onClose={()=>setProfile(false)}/>}</>
}
