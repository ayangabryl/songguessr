...
Reskin check (intake scope rethink or new): capture the build it replaces and the new build at the same viewport, place them side by side, and view at 25 percent zoom (`scripts/capture.mjs` for both; any image viewer at 25 percent, or downscale both to 360 px wide). Record three things: whether the structure differs (regions, order, weights) or only the skin; whether the one idea from `concept.md` can be pointed at in the small image; whether the one new mechanism is present in the render and reachable. The stranger is real: hand the two images, unlabelled and with no concept text, to a second agent (or the client) and ask for one line per image and whether they are the same product; the maker's own line does not count, because the maker always sees the idea in the render. Record the blind lines verbatim. Ask the stranger a fifth question, which of the two looks made by a top studio and why, blunt; its answer is a finding for the next loop, not a compliment to keep. Play the build with sound and motion on (a recorded run, or a mid-playback capture) before writing this report: the motion audit counts animations and cannot see an indicator drawn in the colour of the thing it crosses. If the two lines describe the same product, the build is a reskin. Also measure viewport use at 1440: the width of the main region divided by the viewport; under 60 percent without a reason in `concept.md` is a failed K check. This feeds category K; the metric floor above must still be met, so K never excuses a density regression and J never excuses a reskin. `[house]`
...
| K Reskin test (rethink and new only; n/a on refresh) | 1.5 | blind stranger lines differ (scored by a second agent, not the maker); structure differs, not only skin, at 25 percent side by side; the one idea is legible at that zoom; the one new mechanism is in the render and serves the primary task; the built structure is the chosen divergence option; main region uses at least 60 percent of a 1440 viewport or the concept says why. Under 7 is S1 on a rethink |
