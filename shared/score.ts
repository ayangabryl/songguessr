/** Scoring contract shared by solo, multiplayer and the authoritative Worker. */
export const DEFAULT_SCORE_STAGES = [0.1, 0.5, 2, 8, 15] as const
...
export function enabledStops(stages: readonly number[]): number[] {
  const unique = [
...
export function maxRoundPoints(stages: readonly number[]): number {
...
export function scoreScaleHint(stages: readonly number[]): string {
